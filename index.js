...
class Student {
    ...
    id; //The id property is the student's ID number.
    name; //The name property is the student's name.
    courses; //The courses property is an array of the courses that the student is enrolled in.
    balance; //The balance property is the student's tuition balance.
    constructor(name) {
...
(async () => {
    await showBanner("Student Management System", "Welcome to the Student Management System", "red");
})();
const students = []; //store students here
...
async function viewStudentBalance() {
    ...
    await inquirer //uses the inquirer module to prompt the user for the student's ID
        ...
        if (student) {
            ...
            console.log(`Student ${student.getName()}'s Balance: $${student.viewBalance()}`);
        }
        else {
...
async function enrollInCourse(student) {
    ...
    await inquirer // uses the inquirer module to prompt the user for the course name. The prompt method returns a promise
        ...
        `Student ${student.getName()} has been enrolled in course: ${enrollcourse.course}`);
        mainMenu();
    });
}
...
async function showAllStudentsStatus() {
...
async function mainMenu() {
    ...
    await inquirer // uses the inquirer module to prompt the user for an action
        ...
        switch (method.action //the callback function will execute the action that the user has chosen.
        ) {
            case "Enroll Student":
                enrollStudent();
                break;
            case "Enroll Course": //in this case by using prompt through studentid and then find student using find method and then enroll a student in that course
                ...
                const student = students.find((s) => s.getId() === +studentId.studentId);
                if (student) {
